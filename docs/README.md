...
To start working RSB for your project - copy/paste below prompt to
Claude Code in repository. No worries with missing plan/backlog - RSB
will create templates for you.

    Work on this directory / project as main goal, by using agentic instructions and files from RUPStrikesBack project.

    How to proceed - all instructions, rules, etc. are taken from RUPStrikesBack. Add RUPStrikesBack `https://github.com/rstyczynski/RUPStrikesBack.git` as a submodule, refresh if needed (git submodule update --remote), and copy AGENTS_SUBMODULE.md and HUMANS_SUBMODULE.md from RUPStrikesBack to root of this repository as AGENTS.md and HUMANS.md (remove _SUBMODULE suffix). Then run `@RUPStrikesBack/.claude/commands/rup-manager.md`.
...
requests `implementation`, and the Agent builds the solution following
the test specifications created during Elaboration. The Constructor
fills in test skeletons but does NOT create new test cases - tests were
already specified in the Design phase. When implementation is complete,
the code proceeds to Quality Gates for validation.
...
flowchart LR
    ...
    subgraph Construction["<b>CONSTRUCTION PHASE</b>"]
        ...
        Co3[Agent fills<br/>test skeletons]
        Co4{Implementation<br/>complete?}

        Co1 --> Co2 --> Co3 --> Co4
        Co4 -->|no| Co2
    end

    Co4 -->|yes| Next([Continue to Quality Gates])

    style Construction fill:#d1ecf1
    style Co2 fill:#ffcccc
    style Co3 fill:#ffcccc
```

## Quality Gates Phase

The Quality Gates phase validates the implementation through automated
testing. This phase executes two categories of tests based on Sprint
configuration: **New-Code Gates (A1-A3)** run tests specified in the
`Test:` parameter, and **Regression Gates (B1-B3)** run tests specified
in the `Regression:` parameter. If tests fail, the Agent fixes the code
and re-runs the failing gate - this fix-and-retry loop continues until
all gates pass or retry limits are reached.

```mermaid
flowchart LR
    From([From Construction]) --> Q1

    subgraph QualityGates["<b>QUALITY GATES PHASE</b>"]
        direction LR
        Q1[Run New-Code<br/>Gates A1-A3]
        Q2{Gates<br/>pass?}
        Q3[Run Regression<br/>Gates B1-B3]
        Q4{Gates<br/>pass?}
        Q5[Fix code]
    ...
    Q4 -->|yes| Next([Continue to Wrap-up])
...
Test levels configurable per Sprint in `PLAN.md`:

- **A1/B1 Smoke**: Quick sanity checks

- **A2/B2 Unit**: Component-level tests

- **A3/B3 Integration**: End-to-end tests

## Wrap-up Phase

The Wrap-up phase finalizes documentation and ensures traceability. The
Documentor agent validates that README is updated with new features, all
progress files are complete, and the Sprint can be marked as Done. This
phase runs even if Quality Gates failed (to document the failure).

```mermaid
flowchart LR
    From([From Quality Gates]) --> W1

    subgraph Wrapup["<b>WRAP-UP PHASE</b>"]
        direction LR
        W1[Documentor<br/>validates docs]
        W2[Update README<br/>with changes]
        W3[Complete<br/>progress files]
        W4{Documentation<br/>complete?}

        W1 --> W2 --> W3 --> W4
        W4 -->|no| W1
    end

    W4 -->|yes| Final{All phases<br/>implemented?}
    Final -->|no| Back([Return to Inception])
    Final -->|yes| End([Project Finished])

    style Wrapup fill:#d4edda
...
- `progress/sprint_15/sprint_15_setup.md` (Contracting + Inception)

- `progress/sprint_15/sprint_15_design.md` (Elaboration + Test
  Specification)

- `progress/sprint_15/sprint_15_implementation.md` (Construction)

- `progress/sprint_15/sprint_15_tests.md` (Quality Gates)

- `progress/sprint_15/sprint_15_bugs.md` (Bug tracking)

- `progress/sprint_15/sprint_15_documentation.md` (Wrap-up)
