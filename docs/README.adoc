...
To start working RSB for your project - copy/paste below prompt to Claude Code in repository. No worries with missing plan/backlog - RSB will create templates for you.
...
* **Test**: `smoke`, `unit`, `integration`, `none` — quality gates for new code
* **Regression**: `smoke`, `unit`, `integration`, `none` — quality gates for existing code

=== Commands (Version 2.0)

Version 2.0 provides commands for orchestration, phase execution, and lifecycle management.
...
These commands provide structured workflows for backlog item definition, sprint configuration, and bug handling with fold-in/promote policies.
...
[.text-center]
image::Development-iterative.png[Iterative and incremental software development, width=450]
...
[.text-center]
image::oum_scrum_xp.png[RUP, Scrum, and XP combined together, width=300]
...
The construction phase brings the design to life. The Product Owner requests `implementation`, and the Agent builds the solution following the test specifications created during Elaboration. The Constructor fills in test skeletons but does NOT create new test cases - tests were already specified in the Design phase. When implementation is complete, the code proceeds to Quality Gates for validation.
...
The Quality Gates phase validates the implementation through automated testing. This phase executes two categories of tests based on Sprint configuration: **New-Code Gates (A1-A3)** run tests specified in the `Test:` parameter, and **Regression Gates (B1-B3)** run tests specified in the `Regression:` parameter. If tests fail, the Agent fixes the code and re-runs the failing gate - this fix-and-retry loop continues until all gates pass or retry limits are reached.
...
The Wrap-up phase finalizes documentation and ensures traceability. The Documentor agent validates that README is updated with new features, all progress files are complete, and the Sprint can be marked as Done. This phase runs even if Quality Gates failed (to document the failure).
...
* `progress/sprint_15/sprint_15_design.md` (Elaboration + Test Specification)
...
[.text-center]
image::agile_anarchy.png[Agile Anarchy, width=300]
