...
function renderLocation(el,location){if(!location){hide(el);return}const{name,country,admin1,latitude,longitude}=location;el.innerHTML=`<h2 class="text-lg font-semibold mb-2">Location</h2><div>${name||''}${admin1?', '+admin1:''}${country?', '+country:''}</div><div class="text-sm text-gray-600 dark:text-gray-300">Coordinates: ${latitude?.toFixed?.(2)??''}, ${longitude?.toFixed?.(2)??''}</div>`;show(el)}
